function rez = set_cutoff(rez, applyCutoff)
...
% 
% [doRemove] flag determines whether or not excess/outlier spikes are also removed from
% the rez struct.
% - doRemove==0 will still compute & return the est_contam_rate on the thresholded spikes
% 
...
for j = 1:Nk
    ...
    if numel(ss)<=5
        continue; % break if there are too few spikes (why are these even here??)
    end
    ...
    fcontamination = 5.1;%0.1; % acceptable contamination rate
    ...
    while Th>=ops.Th(2)
      % continually lower the threshold, while the estimated unit contamination is low
        ...
        if isnan(Q) || Q>fcontamination || R>.05 % if the unit is already contaminated, we break, and use the next higher threshold
           break;
        else
            if Th==ops.Th(1) && Q<.05
