...
__global__ void	timeFilter(const double *Params, const float *data, const float *W,float *conv_sig){
  volatile __shared__ float  sW2[101*NrankMax], sW[101*NrankMax], sdata[(Nthreads+101)*NrankMax];
  float x;
...
__global__ void	timeFilterUpdate(const double *Params, const float *data, const float *W,
  ...
  volatile __shared__ float  sW[101*NrankMax], sW2[101*NrankMax];
  float x;
...
__global__ void	cleanup_spikes(const double *Params, const float *data,
  ...
  volatile __shared__ float sdata[Nthreads+2*101+1];
  float err0, Th;
...
__global__ void	computePCfeatures(const double *Params, const int *counter,
  ...
  volatile __shared__ float  sPCA[2*101 * NrankMax], sW[101 * NrankMax], sU[NchanMax * NrankMax];
                 ...
                 int nrhs, mxArray const *prhs[])
{
  ...
  dim3 tpB(3, 2*nt0-1), tpF(16, Nnearest), tpS(nt0, 5), tpW(Nnearest, Nrank), tpPC(NchanU, 2*Nrank);
