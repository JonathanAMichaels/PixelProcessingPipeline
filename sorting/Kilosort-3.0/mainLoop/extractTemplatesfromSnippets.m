...
group_percent_expansion = 20; % percent
...
while iter < 2
    ...
    total_cost_for_wave = corr_sum_with_other_template_choices + 4*nPCs*wTEMP_gaussian_residual + 12*nPCs*highest_template_similarity_penalty;
    % else
    ...
    if N_tries_for_largest_CC_idx_so_far >= N_waves_between_choices(largest_CC_idx) || N_tries_for_largest_CC_idx_so_far >= length(idx)
...
% % final check for any highly correlated waveforms
% CC = wTEMP' * wTEMP; % compute cross-channel correlation matrix
% CC = CC - diag(diag(CC)); % set diagonal to zero
% % find any highly correlated waveforms
% [~, idx] = find(abs(CC) > .9);
% % search through all spikes until 
